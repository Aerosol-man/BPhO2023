...
LineGenerator::LineGenerator(QObject *parent, PlanetData *planetData, Orbits *orb)
    : QObject{parent}
{
    data = planetData;
    orbits = orb;
...
QVector<QVector4D> LineGenerator::lines(int p1, int p2, int periods, int samples)
{
    ...
    xt::xtensor<double, 2> outerAngleTimes = AngleIntegrator::integrate(outerPeriod, data->_eccentricities[outer], periods, false, samples);
    xt::xtensor<double, 1> t = xt::col(outerAngleTimes, 0);
    xt::xtensor<double, 1> outerAngles = xt::col(outerAngleTimes, 1);

    xt::xarray<double> innerAngleTimes = AngleIntegrator::integrate(innerPeriod, data->_eccentricities[inner], 1, false, samples);
    ...
    xt::xtensor<double, 1> interpInnerAngles = AngleIntegrator::interpolate(innerTimes, innerAngles, wrappedTimes);
